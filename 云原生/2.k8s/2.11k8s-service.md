...
* clusterlP
  ...
  * 在EAPIServer 启动时，需要通过service －cluster －ip－range 参数配置虚拟IP地址段，AP！ Server 中有用于分配IP地址和端口的组件，当该组件捕获EService 对象并创建事件时，会从配置的虚拟IP地址段中取一个有效的IP地址，分配给该Service 对象。
* nodeport
  ...
  * 每个节点的kube－oroxy 会尝试在服务分配的nodePort 上建立侦听器接收请求，并转发给服务对应的后端Pod实例。
* LoadBalancer
